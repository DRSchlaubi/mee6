...
class Levels(Plugin):
    ...
    @command(pattern="!levels",
             description="Gibt dir den Link zum Leaderboard",
             banned_roles="banned_roles")
    async def levels(self, message, args):
        url = "<http://levels.schlb.pw/levels/" + message.server.id + ">"
        response = "Checke das Leaderboard von  **" + message.server.name + "**ab: "
        response += url + " :wink: "
        await self.mee6.send_message(message.channel, response)

    @command(pattern="(^!rank$)|(^!rank <@!?[0-9]*>$)",
             description="Gibt dir die Statistiken eines Users aus",
             cooldown="cooldown",
             banned_roles="banned_roles")
    async def rank(self, message, args):
        ...
        if message.channel.permissions_for(message.server.me).embed_links:
            ...
            embed.set_footer(text='Mee6.schlb.pw', icon_url=MEE6_ICON)
